# Calculate travel times for all stops, but limited to connections with 0 transfers
...
# existing_files <- list.files(sprintf("data/travel_times_%s_notrans", DAY_NAME), "*.csv", full.names = FALSE)
    ...
    tryCatch(
      {
        ...
        write.csv(
          ...
          sprintf(
            "data/travel_times_%s_notrans/%s.csv",
            DAY_NAME,
...
write.csv(failures_unlist, sprintf("fails_%s_notrans.csv", DAY_NAME))
